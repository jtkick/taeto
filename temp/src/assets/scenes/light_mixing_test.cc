...
void LightMixingTest::load(int window_height, int window_width)
{
    // Load backdrop
    ...
    // Load spotlights
    shared_ptr<Spotlight> red_light =
        make_shared<Spotlight>(Color(255, 0, 0), 0.8, 30);
    red_light->move(0, -7, 0);
    message_bus->post_message(make_shared<LightUpdateMessage>(red_light));

    shared_ptr<Spotlight> green_light =
        make_shared<Spotlight>(Color(0, 255, 0), 0.8, 30);
    green_light->move(-21, 5, 0);
    message_bus->post_message(make_shared<LightUpdateMessage>(green_light));

    shared_ptr<Spotlight> blue_light =
        make_shared<Spotlight>(Color(0, 0, 255), 0.8, 30);
